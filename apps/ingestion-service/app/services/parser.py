...
def _refine_sections(sections: list[ParsedSection], boilerplate_lines: set[str]) -> list[ParsedSection]:
    ...
    for sec in sections:
        ...
        content = (
            sec.content.strip() if is_preamble_block else _remove_boilerplate_phrases(sec.content, boilerplate_lines)
        )
...
def _commit_section(
    ...
    if (
        not content
        and not is_numbered
        and not (HEADING_RE.match(normalized_title) and (normalized_title.istitle() or normalized_title.isupper()))
    ):
        return
    sections.append(
        ParsedSection(
            ...
            if is_heading(line):
                ...
                if NUMBER_ONLY_SUBSECTION_RE.match(normalized_line):
                    ...
                    if (
                        ...
                        and SECTION_HEADING_RE.match(_normalize_section_title(current_title))
                    ):
