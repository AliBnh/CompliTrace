...
def test_not_assessable_gap_is_promoted_to_publishable_substantive_finding():
    with _session() as db:
        ...
        _enforce_review_publish_invariant(db, audit.id, disposition)
        row = next(
            r for r in final_findings_dataset(db, audit.id) if r.section_id == "systemic:missing_retention_period"
        )
...
def test_fallback_evidence_is_readable_sentence():
    with _session() as db:
        ...
        _enforce_review_publish_invariant(db, audit.id, disposition)
        row = next(
            r for r in final_findings_dataset(db, audit.id) if r.section_id == "systemic:purpose_specificity_gap"
        )
        assert row.policy_evidence_excerpt is not None
        assert row.policy_evidence_excerpt.endswith(".")
        assert row.policy_evidence_excerpt.startswith("Based on the reviewed notice")
...
def test_publishable_without_issue_key_is_downgraded_before_api_boundary():
    with _session() as db:
        ...
        assert all(
            not (
                r.artifact_role == "publishable_finding"
                and r.publication_state == "publishable"
                and _finding_issue_id(r) is None
            )
            for r in rows
        )
