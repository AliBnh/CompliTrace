...
from app.services.clients import DocumentData, SectionData
...
def test_generate_report_writes_valid_pdf(tmp_path: Path):
    ...
    try:
        with SessionLocal() as db:
            ...
            assert "CompliTrace GDPR Gap Report" in decoded
            assert "Executive Summary" in decoded
            assert "Document-wide findings" in decoded
    finally:
        ...
        with SessionLocal() as db:
            ...
            db.add(
                Finding(
                    ...
                    status="gap",
                    severity=None,
                    legal_requirement="GDPR Art 13(1)(c)",
            ...
            clean_finding = Finding(
                ...
                severity="high",
                legal_requirement="GDPR Art 13(1)(c)",
                gap_note="The notice does not disclose the lawful basis for processing.",
                remediation_note="Add a lawful basis statement for each processing purpose.",
                primary_legal_anchor='["GDPR Art 13(1)(c)"]',
                publication_state="publishable",
            )
            db.add(clean_finding)
            db.flush()
            db.add(
                FindingCitation(
                    finding_id=clean_finding.id,
                    chunk_id="lb-clean-1",
                    article_number="13",
                    paragraph_ref="1(c)",
                    article_title="Legal basis",
                    excerpt="No explicit disclosure found in this section.",
                )
            )
            db.commit()
