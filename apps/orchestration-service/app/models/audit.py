...
class Audit(Base):
    ...
    document_group_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("document_groups.id", ondelete="SET NULL"), index=True, nullable=True
    )
    ...
    document_group: Mapped["DocumentGroup | None"] = relationship(back_populates="audits")
...
class DocumentGroup(Base):
    ...
    audits: Mapped[list[Audit]] = relationship(back_populates="document_group")
...
class AuditAnalysisItem(Base):
    ...
    citations: Mapped[list["AnalysisCitation"]] = relationship(
        back_populates="analysis_item", cascade="all, delete-orphan"
    )
...
class AnalysisCitation(Base):
    ...
    analysis_item_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("audit_analysis_items.id", ondelete="CASCADE"), index=True
    )
    chunk_id: Mapped[str] = mapped_column(String(128))
    article_number: Mapped[str] = mapped_column(String(32))
    paragraph_ref: Mapped[str | None] = mapped_column(String(64), nullable=True)
...
class RemediationItem(Base):
    ...
    issue_label: Mapped[str] = mapped_column(String(256))
    severity: Mapped[str] = mapped_column(String(16))
    score_impact_points: Mapped[int] = mapped_column(Integer)
    order_index: Mapped[int] = mapped_column(Integer)
    section_id: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    audit: Mapped["Audit"] = relationship(back_populates="remediation_items")
    finding: Mapped["Finding"] = relationship()
    suggestion: Mapped["RemediationSuggestion | None"] = relationship(
        back_populates="remediation_item", cascade="all, delete-orphan", uselist=False
    )
...
class RemediationSuggestion(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    remediation_item_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("remediation_items.id", ondelete="CASCADE"), unique=True, index=True
    )
    suggested_fix_text: Mapped[str | None] = mapped_column(Text, nullable=True)
    generation_status: Mapped[str] = mapped_column(String(16), default="pending")
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    remediation_item: Mapped["RemediationItem"] = relationship(back_populates="suggestion")
